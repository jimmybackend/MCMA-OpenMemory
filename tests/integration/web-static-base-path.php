...
check(str_contains($index,'id="conversationPageUp"'),'conversation page-up control is present');
check(str_contains($index,'id="conversationPageDown"'),'conversation page-down control is present');
...
check(str_contains($app,'loadContext({maxPasses:2})'),'Contexto preload is wired');
check(str_contains($app,"contextRefresh.addEventListener('click'"),'Contexto refresh button is wired');
