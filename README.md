...
MCMA does **not** own or modify the host web-server configuration. The repository ships only a generic, non-production `config/nginx/mcma-web.conf.example` for an operator to review and adapt manually. Nginx/Apache/CDN files, reloads, TLS and virtual-host composition remain the deployment operator's responsibility.
...
Conversation turns are stored once under `memory://interactions/...`; the chat sidebar and library shelves are derived views rather than copies.

Canonical personal-memory details in **Biblioteca** now include **Actualizar en Chat**. The selected `memory://user/...` reference is bound visibly to the current conversation and sent as a separate `mutation_ref`; it takes priority over conversational recency or lexical target resolution. Creating a new conversation clears the binding, and the user can remove it manually from the composer.

Relevant archived interactions now also produce **portable thematic summaries** under `memory://user/temas/<topic-slug>/resumenes/<req-id>`. These derived objects keep only a short summary, validation state, topic/project/entity metadata and the canonical interaction reference; they do not duplicate the full question/answer transcript. Classification reuses existing catalog topics first and deterministic project/entity/canonical-memory signals next, so no extra model call is introduced by this layer. Approval/retraction versions the same summary and only verified summaries are marked `trusted_for_conclusions=true`. A private encrypted conversation index stores only conversation metadata plus canonical interaction references, so normal sidebar rendering does not decrypt the whole library. Listing/opening conversations uses zero AI tokens and zero AI credits. An archived interaction starts unverified; the owner can approve it as reusable Knowledge or retract it. Deep catalog classification runs only on approval and is metered when billing is enabled. The separate Context trace window remains capped to the latest 50 requests for operational transparency, while the durable interaction archive is not capped at 50.
...
It prepares PHP-FPM, protected runtime directories and `/etc/mcma/mcma.env` without overwriting an existing environment file or generating external-provider credentials. It does not install, edit, enable, restart or reload Nginx/Apache/CDN configuration.
